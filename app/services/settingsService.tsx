...
const enablePushNotifications = async ({ userId }: any) => {
    ...
    try {
        ...
        const { data, error } = await supabase
            .from('profiles')
            .select('enablepushnotifications')
            .eq('user_id', userId)
            .single();
        ...
        if (data) {
            // If enablepublicprofile is true, toggle it to false, otherwise set it to true
            const newPushNotificationsStatus = !data.enablepushnotifications;

            // Update the enablepublicprofile field to the toggled value
            const { error: updateError } = await supabase
                .from('profiles')
                .update({ enablepushnotifications: newPushNotificationsStatus })
                .eq('user_id', userId);
            ...
            console.log(`Push notifications ${newPushNotificationsStatus ? 'enabled' : 'disabled'}.`);
            return { message: `Profile ${newPushNotificationsStatus ? 'enabled' : 'disabled'} successfully.` };
        }
...
export { enablePublicProfile, enablePushNotifications };
